...
permalink: /hermes/user-guide/features/rl-training
---

# 强化学习训练

Hermes Agent 包含一个基于 **Tinker-Atropos** 的集成 RL（强化学习）训练管道。这支持在环境特定任务上使用 GRPO（组相对策略优化）和 LoRA 适配器训练语言模型，完全通过 Agent 的工具界面进行编排。

## 概述

RL 训练系统由三个组件组成：

1. **Atropos** —— 一个轨迹 API 服务器，协调环境交互、管理推出组并计算优势
...
Agent 调用 `rl_list_environments()`，它使用 AST 解析扫描 `tinker-atropos/tinker_atropos/environments/` 以查找继承自 `BaseEnv` 的 Python 类。每个环境定义：
...
:::note 速率限制
状态检查限制为每运行 ID 每 **30 分钟** 一次。这防止了在需要数小时的长时间运行训练作业期间的过度轮询。
:::
### 5. 停止或获取结果
...
在提交到完整训练运行之前，您可以使用 `rl_test_inference` 测试环境是否正常工作。这使用 OpenRouter 运行几步推理和评分 —— 不需要 Tinker API，只需要 `OPENROUTER_API_KEY`。
...
- **Training client** —— 处理带重要性采样损失的前向-后向传递、优化器步骤（Adam）和权重检查点
- **Sampling client** —— 使用最新训练权重提供推理

训练循环：
1. 从 Atropos 获取一批推出（提示 + 完成 + 分数）
2. 转换为带有填充对数概率和优势的 Tinker Datum 对象
3. 运行带重要性采样损失的前向-后向传递
...
研究现有的 `gsm8k_tinker.py` 作为模板。Agent 可以帮助您创建新环境 —— 它可以读取现有环境文件、检查 HuggingFace 数据集并编写新环境代码。

## WandB 指标

训练运行记录到 Weights & Biases，包含这些关键指标：
