...
permalink: /hermes/user-guide/features/context-references
---

# 上下文引用

输入 `@` 后跟引用以将内容直接注入到您的消息中。Hermes 内联扩展引用并将内容附加在 `--- Attached Context ---` 部分下。
...
| `@file:path/to/file.py:10-25` | 注入特定行范围（从1开始计数，包含）|
| `@folder:path/to/dir` | 注入带有文件元数据的目录树列表 |
...
- `@file:` 和 `@folder:` 触发带有文件大小元数据的文件系统路径补全
- 裸 `@` 后跟部分文本显示当前目录中匹配的文件和文件夹

## 行范围

`@file:` 引用支持行范围以进行精确内容注入：
...
上下文引用有边界以防止超出模型的上下文窗口：

| 阈值 | 值 | 行为 |
|-----------|-------|----------|
| 软限制 | 上下文长度的 25% | 附加警告，扩展继续 |
| 硬限制 | 上下文长度的 50% | 扩展被拒绝，原始消息原样返回 |
| 文件夹条目 | 最多 200 个文件 | 多余条目替换为 `- ...` |
| Git 提交 | 最多 10 个 | `@git:N` 钳位到范围 [1, 10] |

## 安全

### 敏感路径阻止

这些路径始终被阻止用于 `@file:` 引用以防止凭证暴露：
...
所有路径都相对于工作目录解析。解析到允许工作区根目录之外的引用被拒绝。

### 二进制文件检测

二进制文件通过 MIME 类型和空字节扫描检测。已知的文本扩展名（`.py`、`.md`、`.json`、`.yaml`、`.toml`、`.js`、`.ts` 等）绕过基于 MIME 的检测。二进制文件被拒绝并显示警告。

## 平台可用性

上下文引用主要是一个 **CLI 功能**。它们在 `@` 触发 tab 补全的交互式 CLI 中工作，引用在消息发送到 Agent 之前被扩展。

在**消息平台**（Telegram、Discord 等）上，网关不会扩展 `@` 语法 —— 消息按原样传递。Agent 本身仍然可以通过 `read_file`、`search_files` 和 `web_extract` 工具引用文件。

## 与上下文压缩的交互

当对话上下文被压缩时，扩展的引用内容包含在压缩摘要中。这意味着：

- 通过 `@file:` 注入的大文件内容会贡献到上下文使用
- 如果对话稍后压缩，文件内容会被总结（而非逐字保留）
