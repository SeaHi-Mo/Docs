...
permalink: /hermes/developer-guide/memory-provider-plugin
---

# 构建记忆提供商插件

记忆提供商插件为 Hermes Agent 提供内置 MEMORY.md 和 USER.md 之外的持久、跨会话知识。本指南介绍如何构建一个。
...
| `save_config(values, hermes_home)` | 将非秘密配置写入原生位置 | **是**（除非仅环境变量） |
...
带有 `secret: True` 和 `env_var` 的字段进入 `.env`。非秘密字段传递给 `save_config()`。

:::tip 最小化 vs 完整模式
`get_config_schema()` 中的每个字段在 `hermes memory setup` 期间都会提示。有许多选项的提供商应保持模式最小化 —— 仅包括用户**必须**配置的字段（API 密钥、必需凭证）。在配置文件参考中记录可选设置（例如 `$HERMES_HOME/myprovider.json`），而不是在设置期间全部提示。这保持设置向导快速，同时仍支持高级配置。参见 Supermemory 提供商示例 —— 它只提示 API 密钥；所有其他选项位于 `supermemory.json`。
:::
...
所有存储路径**必须**使用来自 `initialize()` 的 `hermes_home` kwargs，而不是硬编码的 `~/.hermes`：
...
参见 `tests/agent/test_memory_plugin_e2e.py` 获取使用真实 SQLite 提供商的完整 E2E 测试模式。
...
1. 添加 `cli.py` 文件到你的插件目录
2. 定义一个构建 argparse 树的 `register_cli(subparser)` 函数
3. 记忆插件系统通过 `discover_plugin_cli_commands()` 在启动时发现它
4. 你的命令出现在 `hermes <provider-name> <subcommand>` 下

**活动提供商门控：** 当你的提供商是配置中的活动 `memory.provider` 时，你的 CLI 命令才会出现。如果用户尚未配置你的提供商，你的命令不会显示在 `hermes --help` 中。
...
参见 `plugins/memory/honcho/cli.py` 获取包含 13 个子命令、跨配置文件管理（`--target-profile`）和配置读/写的完整示例。
...
一次只能有一个外部记忆提供商处于活动状态。如果用户尝试注册第二个，MemoryManager 会拒绝并发出警告。这防止工具模式膨胀和冲突的后端。
